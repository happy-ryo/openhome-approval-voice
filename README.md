...
> **primary は requests HTTP pull**: PC 側 exporter がキュー JSON を atomic 書き込み → LAN 配信、
> DevKit の ability が poll 毎に **GET のみ**で取得（書き戻さない＝一方向維持）。`requests` は
> add-capability で受理（実測 201）、`urllib`/`http.client`/`socket` は forbidden import で reject。
> **fallback は PC-side push（scp/sftp）**: device で egress できない場合の正式 fallback で、ability は
> ネットワークせず自 storage を読むだけ（PC が push）。`py -3 -m pc_exporter push`（`pc_exporter/push.py`）。
> **device の egress 可否は ≈要検証**（実機 GET で確定）。詳細・出典は `deploy/DEPLOY.md` §4.3 /
> `docs/design.md` §M3.3.1。
