...
### 8.4 スモーク（SSH レス・SMOKE_BOOTSTRAP）
依頼者は DevKit に SSH 不可＝device ログ確認不可。よって**「実際に音が出る」ことが唯一の地上検証**に
なるよう、daemon に **`SMOKE_BOOTSTRAP`**（`transport.py`, 既定 True）を実装:
- enable 済み daemon が起動時、**キュー不在ならサンプルを自己投入**（`background._maybe_bootstrap`）→
  次の poll で 4 ゲートを `speak()`。音声トリガ/main.py 配線/SSH に**一切非依存**（依頼者は聴くだけ）。
- 冪等: キューが在れば再 seed しない。read-cursor で二重読み上げ無し。**本番は `SMOKE_BOOTSTRAP=False`**＝
  seed は bridge/exporter のみ（従来設計）。seed ロジックは `bridge.sample_queue_json()` に一本化
  （main.py の interactive seed と同一・単一真実源）。
- `tests/test_background.py`: fake async `capability_worker` で daemon 経路を実証
  （bootstrap→4 ゲート speak→dedup、interrupt は 1 回）。

手順:
1. `--root-folder approvalvoice` でビルド（sandbox-clean 自動検証）
2. add-capability（`name=approvalvoice` / `category=background_daemon` / `personality_id=590628`）
3. **assign（partial update）**: `PUT /api/personalities/edit-personality/`（`personality_id=590628` ＋
   `matching_capabilities=<cap_id>` の**2項目のみ**。公式 CLI と同形＝description/raw_prompt/voice_id 等は不変）。
   ※ ①の `personality_id` だけでは紐付かない実測あり→この明示 assign が必要。
4. enable は最新版が `is_user_enabled=true` なら不要（`is_committed/published` はマーケット公開用で個人利用に無関係）。
5. ⇒ enable 済 daemon が起動→自己 seed→**4 ゲートを逐語 speak**。最終「逐語で聞こえたか」の試聴のみ依頼者。
