...
# 属性アクセスとしての dunder（`cls.__dataclass_fields__` / `obj.__dict__` 等）を禁止。
# `.` を要求するので `from __future__ import` や `def __post_init__` は誤検出しない。
_DUNDER_ATTR = re.compile(r"\.__\w+__")
            ...
            with open(full, encoding="utf-8") as fh:
                for i, line in enumerate(fh, 1):
                    for rx, label in (
                        ...
                        (_PATHLIB, "pathlib"),
                        (_DUNDER_ATTR, "dunder attribute access"),
                    ):
                        if rx.search(line):
