...
### M3.1-s.4 seed の再配置: main.py の write_file（SSH 不要）

旧 `deploy/seed_queue.py`（端末上で file path にサンプルをコピー）は storage-name モデルでは
ability が読めず無効化。スモークの seed は **interactive `main.py` をトリガすると
`write_file` でサンプル 4 ゲートを storage に書き込み + 既読カーソルを reset** する形へ移した
（SSH/seed_queue 不要、再トリガで毎回フレッシュに 4 件読み上げ）。`seed_queue.py` は削除。
...
sandbox 準拠 + `main.py` seed による on-device スモークまでで、live org state → 端末 storage の
本番配送は **Issue #7（on-device end-to-end）の確定事項**とする。候補は (a) ability 自身が poll
