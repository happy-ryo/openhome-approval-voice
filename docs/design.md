...
> **用語の訂正（実行場所 ≠ "on-device"・2026-06-26 live 統合で確定）**: 本書および
> README で多用する **"(C) DevKit on-device" / "on-device 逐語"** は、**実行場所**を指す
> 表現ではなく、**`speak()` 経由＝LLM を介さず逐語で読み上げる**という**経路**の意味で読む。
> 実測した実行場所は **OpenHome の cloud（Ubuntu サーバ）であって DevKit ハードウェアでは
> ない**: ability コードは cloud 上で実行され、**DevKit はマイク/スピーカの audio I/O 端末**
> として cloud の `speak()` 出力を鳴らす（多重ホスティングで user 毎に scoping される）。
> 根拠は cloud の error log が示す path
> `/home/ubuntu/OpenHome/src/../user_capabilities/<user_id>/...`（`<user_id>` で
> ディレクトリ分離＝サーバ側の多重ホスティング）。したがって §M3.3.1 以下の表で
> consumer（reader）の「動作場所」を **DevKit** と記す箇所は、厳密には **cloud 上の
> ability が実行し、その音声出力先が DevKit** の意味であり、ability の outbound 通信
> （pull の `requests.get`）も **cloud から発する**（→ private LAN IP は不可達。public
> HTTPS が必須。`deploy/DEPLOY.md` §4.3(A)/§6）。本節以降の規範記述は経路・契約の規定で
> あり、この用語注記で実行場所の読み替えのみ補う（規範記述自体は不変）。
...
> ※ 「動作場所＝DevKit」は **音声出力先**の意味。ability コードの**実行場所は cloud**で、
> outbound pull も cloud から発する（§M3.0 の用語訂正を参照）。よって pull 先は private
> LAN IP では不可達で **public HTTPS 必須**（`deploy/DEPLOY.md` §4.3(A)/§6）。
