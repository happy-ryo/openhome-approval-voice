...
This is pure in-memory dedup logic, reused unchanged on both sides. Persistence
of the `seen` cursor is owned by the caller's I/O layer (PC: `approval_voice.fileio`;
on-device: `openhome_ability.background` via `capability_worker`), so this module
stays free of filesystem access and ships safely inside the on-device bundle.
"""
